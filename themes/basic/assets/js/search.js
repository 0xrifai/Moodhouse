...
function executeSearch(term) {
  ...
  if (results.length === 0) { // no results based on what was typed into the input box
    ...
    for (let item in results.slice(0, 5)) { // only show first 5 results
      const title = '<h1 class="text-lg font-bold">' + results[item].item.title + '</h1>';
      const date = results[item].item.date ? '<div><em class="">' + new Date(results[item].item.date).toUTCString().substring(0, 16) + '</em></div>' : '';
      const contents = '<div class="truncate">' + results[item].item.contents + '</div>';

      searchitems = searchitems + '<li><a class="block px-4 py-2 rounded pb-2" href="' + results[item].item.permalink + '" tabindex="0">' + title + date + contents + '</a></li>';
    }
